...
namespace Plugins
{
    internal class CurrentStateColorSlider : BaseCurrentStateEditableGuiEntry<Color>
    {
        ...
        public CurrentStateColorSlider(string name, Func<OCIChar, Color> updateValue, Action<Color> onValueChanged) : base(name, updateValue)
        {
        ...
        protected override GameObject CreateItem(GameObject categoryObject)
        {
            ...
            {
                Singleton<Studio.Studio>.Instance.colorPalette.Setup(Name, Value.Value, c => { OnValueChanged(c); image.color = c; }, true);
